<!-- TODO Weitermachen -->
<template>
  <h2 for="MatchID">1. Bitte gib die ID des Kunden ein</h2>
  <p class="secondtitle">Wenn du Fehler in den Werten unterhalb findest, ändere diese bitte in Bullhorn und gib dann die Kunden-ID erneut ein.<br />Sonst kannst du auch alle Felder gleich im Vertrag in DocuSign ändern.</p>
  <div class="input-group mb-3">
    <input v-model="idFromInput" id="KundeID" type="number" name="Kunden ID" class="form-control" aria-describedby="basic-addon2" @keyup.enter="getKunde()">
    <div class="input-group-append">
      <button class="btn btn-primary" @click="getKunde()"><b>Bestätigen</b></button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getKunde() {
      this.isLoading = true;
      setTimeout(() => {
        BackendService.getBullhornData(this.idFromInput, "kunde").then((response) => {
          try {
            this.kunden = response.data;
            this.confirmClick();

            localStorage.setItem('kunde:kunde', this.matches);
            ...
            localStorage.setItem('customerId','EZ-' + this.idFromInput);
            ...
            alert("Die Kunden-ID ist nicht korrekt. Bitte gib die ID erneut ein.");
            this.isLoading = false;
          }
        });
      }, 3000);
