...
import {
  docxTypeToFields,
  contractTypeToFields,
  fieldNameToDocusignParameterName,
  helpMailFields,
} from "@/stores/PowerFormFields";
...
function constructQueryParams(keys) {
  return keys
    .map(
      (key) =>
        `${
          fieldNameToDocusignParameterName.get(key) || ""
        }=${getEncodedLocalStorageValue(key)}`
    )
    .join("&");
}
...
function generatePowerForm(docId, exceptions) {
  ...
  const additionalParams = exceptions
    .map(([key, value]) => `${key}=${encodeURIComponent(value)}`)
    .join("&");
  const allParams = additionalParams
...
//ToDO: Logik ändern
function getPPName() {
  const ppGesellschaft = localStorage.getItem("ppGesellschaft");
  const projektpartnerName = localStorage.getItem("projektpartnerName");

  return !ppGesellschaft || ppGesellschaft === ""
    ? projektpartnerName
    : `${ppGesellschaft} Projektinhaber: ${projektpartnerName}`;
}
...
function generalDocxOperation(type) {
  ...
  BackendService.postDocData(localStorage.getItem("docId"), data).then(
    (response) => {
      openURL(response.data.toString());
    }
  );
}
